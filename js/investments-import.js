...
function _importBuildPrompt(accounts, snapshotType) {
    ...
    return [
        ...
        accountList,
        '',
        '=== SPREADSHEET STRUCTURE — READ THIS CAREFULLY ===',
        '',
        'The spreadsheet has two distinct sections separated by a blank column:',
        '',
        'SECTION 1 — Summary/category totals (left side):',
        '  Col 1: Date',
        '  Col 2: Net Worth (grand total — the LARGEST number, sum of all accounts)',
        '  Col 3: A second large number — this is the "Invested" total (cost basis). Map it to "ignore".',
        '  Then: standalone category columns with SHORT single-word labels:',
        '    "Roth"      → category:roth      (total of all Roth accounts)',
        '    "PreTax"    → category:preTax    (total of all pre-tax/traditional accounts)',
        '    "Brokerage" → category:brokerage (total of taxable brokerage accounts)',
        '    "Cash"      → category:cash      (total of all bank/cash accounts)',
        '    "Inv Cash"  → category:invCash   (invested/money-market cash)',
        '',
        'SECTION 2 — Individual account columns (right side, after the blank separator):',
        '  The header may span TWO rows: row 1 = person name (Steve, Joint, Connie), row 2 = account name.',
        '  Use the ACCOUNT NAME (row 2) to match against the account list.',
        '  Match each account column to "account:{id}" using the account list above.',
        '',
        '=== CRITICAL NAMING DISTINCTION ===',
        '',
        'The word "Roth" appears in two very different contexts — do NOT confuse them:',
        '  - A column labeled exactly "Roth" (standalone, in the summary section) → category:roth',
        '  - A column labeled "Steve Roth", "Con Roth", "Roth401k" etc. → an ACCOUNT, not a category',
        '',
        'Similarly:',
        '  - Standalone "PreTax" column in the summary section → category:preTax',
        '  - Any account name containing "401k", "Traditional", "PreTax" in the account list → account:{id}',
        '',
        '=== MAPPING RULES ===',
        '',
        '  "netWorth"            → grand total (col 2, largest value)',
        '  "category:roth"       → ONLY the standalone "Roth" summary column',
        '  "category:preTax"     → ONLY the standalone "PreTax" summary column',
        '  "category:brokerage"  → ONLY the standalone "Brokerage" summary column',
        '  "category:cash"       → ONLY the standalone "Cash" summary column',
        '  "category:invCash"    → ONLY the standalone "Inv Cash" summary column',
        '  "account:{id}"        → individual account column matched to account list',
        '  "ignore"              → blank/separator columns, person group header rows (Steve/Joint/Connie), "Invested" total column',
        '',
        'Set uncertain=true when:',
        '  - Multiple accounts could match the column name',
        ...
        '  - Do NOT include a column for the first column (date) — dates go in the rows[].date field'
    ].join('\n');
}
...
function _importRenderGrid() {
    ...
    var categoryOptions = [
        ...
        '<option value="category:roth">Roth (total)</option>',
        '<option value="category:preTax">Pre-Tax (total)</option>',
        '<option value="category:brokerage">Brokerage (total)</option>',
        '<option value="category:cash">Cash (total)</option>',
        '<option value="category:invCash">Inv Cash (total)</option>',
